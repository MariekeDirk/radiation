...
#Create Folds
time.folds<-createFolds(timestamps,2)

time.trainFolds<-time.folds[[1]]
time.testFolds<-time.folds[[2]]

time.trainSet<-timestamps[time.trainFolds]
time.testSet<-timestamps[time.testFolds]

print(head(time.trainSet))
print(head(time.testSet))

#Time slices
I.Sub.TimeSlices<-createTimeSlices(y=as.vector(timestamps),initialWindow = 32,horizon=32,fixedWindow = FALSE)

str(I.Sub.TimeSlices,max.level=1)


time.trainSlices<-I.Sub.TimeSlices[[1]]
time.testSlices<-I.Sub.TimeSlices[[2]]

time.trainSet<-timestamps[time.trainSlices[[1]]]
time.testSet<-timestamps[time.testSlices[[1]]]
