...
obs.df$year<-year(obs.df$IT_DATETIME)
setkey(obs.df, day)
...
for(i in 1:30) seeds[[i]]<- sample.int(n=1000, 30) #(3 is the number of tuning parameter, mtry for rf, here equal to ncol(iris)-2)
seeds[[31]]<-sample.int(1000, 1)
#control list
control<-trainControl(method="cv",repeats=3,index=createFolds(obs.df$Q),seeds=seeds) #setting a 10 fold-cross validation (best performing)
...
fuzzyGridReduced <- expand.grid(num.labels=3:10,max.iter=4:6) #num.labels=3:10,max.iter=4:6
```
