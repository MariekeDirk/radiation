...
ked_exp.cv <- autoKrige.cv(REH1.Q~var, var, model = c("Exp"),maxdist=mxdkrige,fix.values=c(NA,NA,NA), miscFitOptions = list(merge.small.bins = TRUE))
...
```
#Caret package: testing
```{r}
m1.caret<-train(REH1.Q~layer,data=ov,method="glm")

#2 ways of predicting
p1.caret<-predict(m1.rpart,grid)
p1.caret<-predict.train(m1.rpart,newdata=grid)
```
